$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '账号', name: 'studentNo', index: 'student_no', width: 120 }, 			
			...
			{ label: '操作', name: 'id', index: 'passwordd', width: 80,formatter :function(r){
				 return '<button onclick="showimage(' + r +')">头像上传</button><br><button onclick="bindEpc(' + r +')">EPC绑定</button>';
			} }
        ],
		viewrecords: true,
        height: 600,
        ...
        postData:{'schoolId': $("#classId").val(),'userType':2,'classId':null},
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			...
			var url = vm.student.id == null ? "../sys/uniform/student/save" : "../sys/uniform/student/update";
			vm.student.schoolId = $("#classId").val();
			vm.student.userType = 2;
			$.ajax({
				type: "POST",
			    url: url,
