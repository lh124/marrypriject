	...
	$(function(){
		...
		$.ajax({
		    ...
		    success: function(result){
				if(result.status == 'ok'){
					if (result.list.img.length > 0) {
						var content = "";
						var container = $("#photoContainer");
						for(var i = 0; i < result.list.img.length; i++){
							content = '<img origile="' +result.list.img[i].pic + '" '+
										'src="' + result.list.img[i].pic + '" class="img-border img-max img-radius">'+
										'<br>';
