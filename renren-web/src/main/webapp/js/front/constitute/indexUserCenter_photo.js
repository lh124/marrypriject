$(function(){
			$.ajax({
			  type: 'GET',
			  url: './Photofrontuser/userFunction',
			  dataType: "json",
			  success: function(result){
					...
					$("#headImg").attr("src", url + result.user.headImg);
					var content = "";
					// 添加功能
					...
					// 取消微信绑定
					if (result.user.openid != null && result.user.openid != ""){
					...
					// 班级管理
					if (result.admin > 0){
						var htmlT = '<a id="cancelBinding" class="weui-cell weui-cell_access" href="user/manageMyClasses.html">'+
										'<div class="weui-cell__hd"><img src="/statics/images/userCenter/classManage.png" alt="" style="width:20px;margin-right:5px;display:block"></div>'+
										'<div class="weui-cell__bd">'+
											'<p>班级管理</p>'+
										'</div>'+
										'<div class="weui-cell__ft"></div>'+
...
function cancelWechatBinding(){
	$.ajax({
		  type: 'GET',
		  url: './Photofrontuser/cancelWechatBinding',
		  dataType: "json",
		  success: function(result){
