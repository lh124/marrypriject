...
$(function () {
    var pageManager = {
        ...
        _back: function (config) {
            ...
            if (!found) {
                ...
                // alert(stack.dom);
                // 这里发生了一个问题，导致了加入节点会把自身加到自己前面，导致null异常
    ...
    function setPageManager(){
        ...
        pageManager
            ...
            }).setDefault('usercenter').init();
			
			var url = window.location.href;
			
			if(url.indexOf('#')=== 0){
				pageManager.go("usercenter");
			}else{
				var target = url.substr(url.indexOf('#'));
				pageManager.go("usercenter");
				//pageManager.setContainer($("#innerContainer"));
				//pageManager.go(target);
			}
			//alert(url);
			//pageManager.go("tabbar");
            //.setDefault('tabbar')
            //.init();
    }
