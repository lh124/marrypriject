$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '拓展id', name: 'typeId', index: 'type_id', width: 80 }			
        ],
		viewrecords: true,
        height: 385,
        rowNum: 10,
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.photoPicHead.id == null ? "../photopichead/save" : "../photopichead/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.photoPicHead),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(id){
			$.get("../photopichead/info/"+id, function(r){
                vm.photoPicHead = r.photoPicHead;
            });
		},
