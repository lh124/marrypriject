...
function getUserTypes(){
	...
	$.ajax({
		type: "GET",
	    url: "../phototype/querClassType?classId=" + $('#classId').val(),
	    async: false,
	    dataType: "json",
	    success: function(r){
	    	ss = r;
	...
	if(r.status == 'ok'){
		if ( r.list.length > 0 ) {
			for(var i = 0; i < r.list.length; i++){
				var ele = {};
				ele.label = r.list[i].title;
				ele.value = r.list[i].id
				array[i] = ele;
				
				if(i == 0) {
					$('#selectedId').val(r.list[i].id)
					$('#typeName').text(r.list[i].title);
				}
			}
		} else {
			alert('请先添加分类');
			window.location.href = './photoTypeManage.html';
		}
		// 注册事件
	}
	    ...
	    weui.picker(
	        ...
	        onConfirm: function (result) {
	        	...
	        	for(var i = 0; i < array.length; i++){
	        		...
	        		if (array[i].value == result[0]) {
	        			$('#typeName').text(array[i].label);
	        		}
...
function uploaddingShowEnd(){
    ...
    /*setTimeout(function () {
    	window.location.href = "myPhotoType.html";
    }, 3000);*/
}
