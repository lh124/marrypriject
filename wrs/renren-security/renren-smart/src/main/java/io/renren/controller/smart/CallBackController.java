	...
	private void schoolNoticePic(JSONObject json){
    	...
    	if (id != null) {
    		...
    		if(schoolNotice != null){
    			SchoolNoticeEntity schoolNoticeUpdate = new SchoolNoticeEntity();
    			schoolNoticeUpdate.setId(schoolNotice.getId());
    			schoolNoticeUpdate.setNoticepic(ControllerConstant.CDN_URL + json.getString("filename"));
    		}
    	} else {
    		logger.error("智能校服学生头像图片回调关联数据不存在");
			logger.error(json.toString());
