			...
			if (type == null){
				...
				try{
					...
					if (type.equals(TypeEnum.PHOTO_CLASS.getType())){
			        	...
			        	StudentEntity studnet = this.studentService.selectById(jsona.getInt("user_id"));
			        	if(studnet != null){
			        		StudentEntity studentUpdate = new StudentEntity();
			        		studentUpdate.setId(studnet.getId());
			        		studentUpdate.setPic(jsona.getString(FILE_NAME));
			        	}
