    	    	...
    	    	if(r.status == 'ok'){
    	    		if (r.page.list.length > 0) {
	    	    		for (var i = 0; i < r.page.list.length; i++) {
	    	    			result += '<option value="'+ r.page.list[i].id +'">' +r.page.list[i].schoolName +'</option>';
	    	    		}
...
function doManageSchool(){
	var para = "?shoolId=" + $("#school").val() + "&adminId=" + $("#adminId").val();
	confirm('确定提交', function(){
		$.ajax({
			type: "GET",
		    url: "../sys/sysadminschool/save" + para,
		    success: function(r){
				if(r.code == 0){
					alert('操作成功', function(index){
						$('#myModal').modal('hide');
