		...
		try {
			...
			if (msgType.equals(MessageUtil.REQ_MESSAGE_TYPE_EVENT)) {
				...
				if(content.indexOf("登录") > -1){
					textMessage.setContent("<a href='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxb9072ff1ebcf745c&redirect_uri=http%3a%2f%2fwrs.gykjewm.com%2ffront%2flogin.html&response_type=code&scope=snsapi_base&state=1#wechat_redirect'>点击登录学生相册</a>\n"
					...
					textMessage.setContent("Tel：0851-86815155\n"
							                + "QQ：1628650565");
				}else if(content.indexOf("位置") > -1){
					textMessage.setContent("贵州省贵阳市观山湖区会展城101大厦A座11-13"
					...
					textMessage.setContent("点击下面相应的菜单模块进行操作。");
				}
				respXml = MessageUtil.messageToXml(textMessage);
			}
		} catch (Exception e) {
